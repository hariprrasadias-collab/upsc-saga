        ...
        <div className={`pomodoro-widget ${className || ''} ${isFullscreen ? 'fullscreen' : ''}`}>
            ...
            <div className="pomodoro-header">
                ...
                <div className="pomodoro-controls">
                    <button onClick={() => setIsFullscreen(!isFullscreen)} className="settings-btn" aria-label={isFullscreen ? "Exit Fullscreen" : "Fullscreen"} title={isFullscreen ? "Exit Fullscreen" : "Fullscreen"}>
                        <span aria-hidden="true">{isFullscreen ? '↙️' : '⛶'}</span>
                    </button>
                    {isFullscreen && (
                        <button onClick={() => setIsFullscreen(false)} className="settings-btn close-fullscreen" aria-label="Close Fullscreen" title="Close">
                            <span aria-hidden="true">✕</span>
                        </button>
                    )}
                    <button onClick={() => setShowHistory(!showHistory)} className="settings-btn" aria-label="Toggle History" title="History">
                        <span aria-hidden="true">📊</span>
                    </button>
                    <button onClick={() => setShowSettings(!showSettings)} className="settings-btn" aria-label="Toggle Settings" title="Settings">
                        <span aria-hidden="true">⚙️</span>
                    </button>
                    <button onClick={() => setIsMinimized(true)} className="minimize-btn" aria-label="Minimize Timer" title="Minimize">
                        <span aria-hidden="true">−</span>
                    </button>
                </div>
            </div>
                            ...
                            <>
                                ...
                                {!isRunning && (
                                    <button className="edit-time-btn" onClick={() => {
                                        ...
                                        setIsEditing(true);
                                    }} aria-label="Edit Timer" title="Edit Timer">
                                        <span aria-hidden="true">✎</span>
                                    </button>
                                )}
                            </>
