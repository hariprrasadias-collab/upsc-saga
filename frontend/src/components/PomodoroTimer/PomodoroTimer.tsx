    ...
    if (isMinimized) {
        return (
            <div
                ...
                role="button"
                tabIndex={0}
                aria-label="Maximize Pomodoro Timer"
                onKeyDown={(e) => {
        ...
        <div className={`pomodoro-widget ${className || ''} ${isFullscreen ? 'fullscreen' : ''}`}>
            ...
            <div className="pomodoro-header">
                ...
                <div className="pomodoro-controls">
                    ...
                    {isFullscreen && (
                        <button aria-label="Close" onClick={() => setIsFullscreen(false)} className="settings-btn close-fullscreen" title="Close">
                            <span aria-hidden="true">✕</span>
                        </button>
                    )}
                    <button aria-label="History" onClick={() => setShowHistory(!showHistory)} className="settings-btn" title="History">
                        <span aria-hidden="true">📊</span>
                    </button>
                    <button aria-label="Settings" onClick={() => setShowSettings(!showSettings)} className="settings-btn" title="Settings">
                        <span aria-hidden="true">⚙️</span>
                    </button>
                    <button aria-label="Minimize Timer" onClick={() => setIsMinimized(true)} className="minimize-btn" title="Minimize">
                        <span aria-hidden="true">−</span>
                    </button>
                </div>
            </div>
                ...
                {isEditing ? (
                    <div className="timer-edit-overlay">
                        ...
                        <div className="timer-edit-actions">
                            <button aria-label="Save Timer Edit" onClick={handleEditSave} className="save-btn">
                                <span aria-hidden="true">✓</span>
                            </button>
                            <button aria-label="Cancel Timer Edit" onClick={() => setIsEditing(false)} className="cancel-btn">
                                <span aria-hidden="true">✕</span>
                            </button>
                        </div>
                    </div>
