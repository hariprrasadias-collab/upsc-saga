...
const PomodoroTimer: React.FC<PomodoroTimerProps> = ({ className }) => {
    ...
    if (isMinimized) {
        return (
            <div className={`pomodoro-minimized ${className || ''}`} onClick={() => setIsMinimized(false)} role="button" aria-label="Maximize timer" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setIsMinimized(false); } }}>
        ...
        <div className={`pomodoro-widget ${className || ''} ${isFullscreen ? 'fullscreen' : ''}`}>
            ...
            <div className="pomodoro-header">
                ...
                <div className="pomodoro-controls">
                    ...
                    <button onClick={() => setShowHistory(!showHistory)} className="settings-btn" title="History" aria-label="View history">📊</button>
