...
const SyllabusTracker: React.FC<SyllabusTrackerProps> = ({
  ...
  const analytics: Analytics | null = useMemo(() => {
    ...
    topics.forEach((t) => {
      ...
      breakdownMap[t.paper][t.status] =
        (breakdownMap[t.paper][t.status] || 0) + 1;
    });

    const totals = Object.entries(totalsMap).map(([paper, total]) => ({
      paper,
      total,
    }));
  ...
  const saveNotes = async () => {
    ...
    setTopics((prev) =>
      prev.map((t) =>
        t.id === currentTopicId ? { ...t, notes: notesText } : t,
      ),
    );
  ...
  const handleBrainAudit = async () => {
    ...
    try {
      ...
      const context = {
        analytics: analytics,
        weak_areas: analytics.breakdown
          .filter((b) => b.status === "Not Started" && b.count > 10)
          .map((b) => b.paper),
        completion_rates: analytics.totals.map((t) => {
          const completed =
            analytics.breakdown.find(
              (b) => b.paper === t.paper && b.status === "Completed",
            )?.count || 0;
      ...
      setBrainInsight(
        "Strategos is currently unavailable. Please try again later.",
      );
    } finally {
  ...
  const handlePrioritize = async () => {
    ...
    try {
      const result = await brainService.executeAction(
        "PRIORITIZE_SYLLABUS",
        {},
      );
      if (result.success) {
        ...
        alert(
          `Strategos has identified ${result.priority_ids.length} high-yield topics.`,
        );
      } else {
    ...
    <div className="syllabus-container">
          ...
          .map(([paper, subjects]) => (
            <div key={paper} className="paper-section">
              <h2 style={{ margin: 0 }}>
                ...
                >
                  <span className="paper-title-text">{paper}</span>
                      ...
                      return (
                        <div key={subjectKey} className="subject-item">
                          <button
                            ...
                            aria-controls={`section-${subjectKey}`}
                          >
                            <span>{subject}</span>
      ...
      {(brainInsight || isBrainLoading) && (
        <div
          ...
          onClick={() => !isBrainLoading && setBrainInsight(null)}
        >
          <div
            className="notes-modal brain-modal"
