...
def google_auth():
    if not os.path.exists(CREDENTIALS_FILE):
        return (
            jsonify(
                {
                    "error": "credentials.json not found. Please configure Google Cloud credentials."
                }
            ),
            404,
        )
...
def update_google_event(event_id):
    ...
    try:
        ...
        if "start_time" in data:
            event["start"] = {"dateTime": data["start_time"], "timeZone": timezone}
        if "end_time" in data:
            event["end"] = {"dateTime": data["end_time"], "timeZone": timezone}
...
def delete_google_event(event_id):
    ...
    try:
        ...
        conn = get_db()
        conn.execute(
            "DELETE FROM calendar_event_metadata WHERE event_id = ?", (event_id,)
        )
        conn.commit()
...
def event_metadata(event_id):
    ...
    if request.method == "GET":
        ...
        if metadata:
            ...
            return jsonify(
                {"xp_reward": 0, "associated_stat": None, "is_completed": False}
            )
