...
def get_syllabus():
    """Get all syllabus topics with revision info (excluding full notes for performance)"""
    try:
        conn = get_db()
        # Optimized query: Select specific columns and check if notes exist instead of fetching full text
        topics = conn.execute('''
            SELECT
