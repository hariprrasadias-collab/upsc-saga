...
def get_syllabus():
    ...
    try:
        conn = get_db()
        # Optimization: Exclude heavy 'notes' column from list view
        topics = conn.execute('''
            SELECT t.id, t.subject, t.topic, t.sub_topic, t.status, t.paper, t.last_updated,
...
def get_topic_details(id):
    ...
    try:
        conn = get_db()
        # Fetch full row including notes
