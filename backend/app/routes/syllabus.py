...
def get_syllabus():
    """Get all syllabus topics with revision info - Optimized to exclude notes body"""
    try:
        conn = get_db()
        # Explicit column selection to avoid fetching potentially large 'notes' text
        topics = conn.execute('''
            SELECT
                t.id, t.paper, t.subject, t.topic, t.sub_topic, t.status, t.last_updated,
                CASE WHEN t.notes IS NOT NULL AND length(t.notes) > 0 THEN 1 ELSE 0 END as has_notes,
