...
def get_due_cards():
    ...
    try:
        ...
        query = '''
            ...
            FROM flashcards f
            LEFT JOIN (
                -- OPTIMIZATION: Removed O(N^2) IN(SELECT MAX) subquery
                -- SQLite's bare columns feature automatically pulls other columns (halflife, etc.)
                -- from the row matching the MAX() aggregate without needing a self-join.
    ...
    try:
        ...
        all_cards = conn.execute('''
            ...
            LEFT JOIN (
                ...
                -- SQLite's bare columns feature automatically pulls other columns (halflife, etc.)
                -- from the row matching the MAX() aggregate without needing a self-join.
                SELECT flashcard_id, halflife, alpha, beta, MAX(reviewed_at)
                FROM review_sessions
                GROUP BY flashcard_id
            ) rs ON f.id = rs.flashcard_id
        ''').fetchall()
