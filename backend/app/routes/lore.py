...
def handle_lore():
    ...
    if request.method == 'GET':
        ...
        d = request.get_json()
        conn.execute('INSERT INTO lore_tablets (user_id, title, content) VALUES (?, ?, ?)',
                     (user_id, d.get('title'), d.get('content')))
        conn.commit()
...
def mod_lore(nid):
    ...
    # Verify ownership before modifying/deleting
    item = conn.execute('SELECT id FROM lore_tablets WHERE id=? AND user_id=?', (nid, user_id)).fetchone()
    if not item:
        return jsonify({"error": "Not found or unauthorized"}), 403

    if request.method == 'PUT':
        d = request.get_json()
        conn.execute('UPDATE lore_tablets SET title=?, content=? WHERE id=? AND user_id=?',
                     (d['title'], d['content'], nid, user_id))
    else:
