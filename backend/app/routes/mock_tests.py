...
def submit_attempt(attempt_id):
    ...
    try:
        ...
        # ⚡ Bolt: Fix N+1 query problem by batching database updates for correct/incorrect answers
        # using executemany, significantly reducing DB roundtrips.
        if correct_updates:
            conn.executemany(
...
def get_attempt_results(attempt_id):
    ...
    try:
        ...
        return jsonify({
            ...
            'accuracy': round(
                (attempt['total_correct'] / max(1, attempt['total_correct'] + attempt['total_incorrect'])) * 100, 1
            ),
...
def create_test():
    ...
    try:
        ...
        # ⚡ Bolt: Fix N+1 query problem by batching database inserts for test questions using executemany,
        # improving test creation speed.
        if question_tuples:
            conn.executemany('''
                INSERT INTO test_questions (
                    test_id, question_number, question_text, option_a, option_b,
