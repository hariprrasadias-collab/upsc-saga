...
def get_tests():
    ...
    try:
        conn = get_db()
        tests = conn.execute(
            'SELECT * FROM mock_tests ORDER BY created_at DESC').fetchall()
...
def get_test(test_id):
    ...
    try:
        conn = get_db()
        test = conn.execute(
            'SELECT * FROM mock_tests WHERE id = ?', (test_id,)).fetchone()
        if not test:
            return jsonify({'error': 'Test not found'}), 404

        questions = conn.execute(
            'SELECT * FROM test_questions WHERE test_id = ? ORDER BY question_number', (test_id,)).fetchall()
...
def submit_test(test_id):
    ...
    try:
        ...
        # Calculate score
        questions = conn.execute(
            'SELECT id, correct_answer FROM test_questions WHERE test_id = ?', (test_id,)).fetchall()
        total = len(questions)
        correct = 0
...
def submit_attempt(attempt_id):
    ...
    try:
        ...
        attempt = conn.execute(
            'SELECT * FROM test_attempts WHERE id = ?', (attempt_id,)).fetchone()
        if not attempt:
        ...
        # Optimized: Gather IDs to perform bulk DB updates using executemany, preventing N+1 queries.
        correct_updates = []
        incorrect_updates = []
        ...
        accuracy = (correct / (correct + incorrect)) * \
            100 if (correct + incorrect) > 0 else 0
...
def create_test():
    ...
    try:
        ...
        # Optimized: Gather question data to perform bulk DB inserts using executemany, preventing N+1 queries.
        question_inserts = []
