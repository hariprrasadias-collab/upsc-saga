...
def upd_codex():
    ...
    d = request.get_json()
    conn = get_db()
    conn.execute('INSERT INTO user_progress (user_id, node_id, status) VALUES (?, ?, ?) ON CONFLICT(user_id, node_id) DO UPDATE SET status=excluded.status', (user_id, d['node_id'], d['status']))
    conn.commit()
