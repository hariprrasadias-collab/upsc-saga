...
def get_questions():
    ...
    try:
        ...
        years = request.args.getlist('years') or (
            [request.args.get('year')] if request.args.get('year') else [])
        subjects = request.args.getlist('subjects') or (
            [request.args.get('subject')] if request.args.get('subject') else [])
        topics = request.args.getlist('topics') or (
            [request.args.get('topic')] if request.args.get('topic') else [])
...
def get_filters():
    ...
    try:
        ...
        years = conn.execute(
            "SELECT DISTINCT year FROM pyq_questions ORDER BY year DESC").fetchall()
...
def get_topics():
    ...
    try:
        ...
        return jsonify(
            [{'topic': row['topic'], 'subject': row['subject']} for row in topics])
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def toggle_favorite(id):
    ...
    try:
        ...
        curr = conn.execute(
            "SELECT is_favorite FROM pyq_questions WHERE id = ?", (id,)).fetchone()
        if not curr:
        ...
        conn.execute(
            "UPDATE pyq_questions SET is_favorite = ? WHERE id = ?", (new_status, id))
        conn.commit()
...
def ask_strategos(question_id):
    ...
    try:
        ...
        if cache.get(cache_key):
            return jsonify(
                {'success': False, 'error': 'Strategos is thinking. Please wait 5 seconds.'}), 429
        ...
        question = conn.execute(
            "SELECT * FROM pyq_questions WHERE id = ?", (question_id,)).fetchone()
        ...
        **OUTPUT:**
        ...
        # Let's stick to BrainService for consistency but upgrade the payload
        # intent.
        payload = {
...
def create_mock_from_filters():
    ...
    try:
        ...
        if not questions:
            return jsonify(
                {'error': 'No questions found matching filters'}), 400

        # 2. Create Mock Test
        title = "PYQ Archive: " + \
            " - ".join(title_parts) if title_parts else "PYQ Archive: All Questions"
        description = f"Generated from Archives with {
...
def save_answer(session_id):
    ...
    try:
        ...
        is_correct = (selected_answer ==
                      question['correct_option']) if selected_answer else False
...
def get_similar_questions(question_id):
    ...
    try:
        ...
        question = conn.execute(
            "SELECT question_text, subject FROM pyq_questions WHERE id = ?",
            (question_id,
             )).fetchone()
        if not question:
            return jsonify({'error': 'Question not found'}), 404
        ...
        words = [w for w in clean_text.split() if len(
            w) > 4][:10]  # Take top 10 long words
        ...
        try:
            ...
            fallback = conn.execute(
                "SELECT * FROM pyq_questions WHERE subject = ? AND id != ? ORDER BY RANDOM() LIMIT 5",
                (question['subject'],
