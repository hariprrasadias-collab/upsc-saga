...
def consult_the_seer():
    ...
    # Optimize: single query to fetch sum of XP grouped by due_date
    xp_sums_query = conn.execute('''
    ...
    xp_map = {row['due_date']: row['total_xp'] for row in xp_sums_query}
...
def get_year_trends():
    ...
    try:
        ...
        # Optimize: single query to fetch all counts grouped by year and subject
        ...
        year_subject_map = defaultdict(dict)
        for row in all_counts:
            year_subject_map[row['year']][row['subject']] = row['count']
        ...
        for year_row in years:
            ...
            count_map = year_subject_map.get(year, {})
