...
def consult_the_seer():
    ...
    # [Performance] Batch fetch XP to prevent N+1 queries loop
    ...
    xp_map = {row['due_date']: (row['total_xp'] if row['total_xp'] is not None else 0) for row in xp_rows}
...
def get_year_trends():
    ...
    conn = get_db()
    try:
        # [Performance] Batch fetch counts to prevent N+1 queries loop
        ...
        # Build mapping for O(1) lookups and track unique dimensions
        years_set = set()
        subjects_set = set()
        count_map = {}
        ...
        for row in all_counts:
            ...
            if year not in count_map:
                count_map[year] = {}
            count_map[year][subject] = row['count']

        # Ensure consistent sorted ordering
        sorted_years = sorted(list(years_set))
        sorted_subjects = sorted(list(subjects_set))
