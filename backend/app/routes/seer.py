...
def get_year_trends():
    ...
    conn = get_db()
    try:
        # ⚡ Bolt optimization: Fetch all counts simultaneously to avoid N+1 query bottlenecks
        ...
        # Group results in a local Python dictionary for O(1) lookups
        ...
        for row in all_counts:
            y = row['year']
            s = row['subject']
            year_subject_counts[y][s] = row['count']
            unique_years.add(y)
            unique_subjects.add(s)

        data = []
        # Sort extracted dimension sets to guarantee consistent ordering for the frontend
