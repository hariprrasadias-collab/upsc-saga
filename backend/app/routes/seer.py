...
def consult_the_seer():
    ...
    # Fetch all XP history in a single bounded query to avoid N+1 issues
    xp_sum_rows = conn.execute('''
    ...
    xp_map = {row['due_date']: (
        row['total_xp'] if row['total_xp'] is not None else 0) for row in xp_sum_rows}
...
def get_year_trends():
    ...
    conn = get_db()
    try:
        # Fetch all counts simultaneously using a single query to avoid N+1
        # issues
        rows = conn.execute('''
            SELECT year, subject, COUNT(*) as count
            FROM pyq_questions
            WHERE year IS NOT NULL AND subject IS NOT NULL
        ...
        for year in sorted_years:
            ...
            for subject in sorted_subjects:
                year_data[subject] = trends[year].get(subject, 0)
