...
def get_year_trends():
    ...
    try:
        ...
        data = []
        # Get counts for all years and subjects efficiently
        counts = conn.execute('''
        ...
        # Build a nested dictionary for fast lookup: count_map[year][subject] = count
        count_map = {}
        for row in counts:
            y = row['year']
            if y not in count_map:
                count_map[y] = {}
            count_map[y][row['subject']] = row['count']
