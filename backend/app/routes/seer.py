...
def consult_the_seer():
    ...
    # Sum XP of tasks completed in the last 7 days (Approximation)
    # Note: ideally we track 'completed_at' timestamp, but using due_date for now is a safe fallback
    date_7_days_ago = (today - datetime.timedelta(days=6)).isoformat()
    ...
    xp_history_query = '''
        ...
        GROUP BY due_date
    '''
    xp_rows = conn.execute(xp_history_query, (user_id, date_7_days_ago, today_str)).fetchall()
