...
def get_year_trends():
    ...
    conn = get_db()
    try:
        # Bolt optimization: Replaced N+1 queries in loop with a single GROUP BY query
        ...
        # Get all counts grouped by year and subject in one query
        counts = conn.execute('''
        ...
        for row in counts:
            ...
            if year not in year_counts:
                year_counts[year] = {}
            year_counts[year][row['subject']] = row['count']
        ...
        for year_row in years:
            ...
            count_map = year_counts.get(year, {})
