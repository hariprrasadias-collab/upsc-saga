...
def get_autonomy_settings():
    ...
    try:
        ...
        level = autonomy_manager.get_user_autonomy_level(user_id=user_id)
        stats = autonomy_manager.get_autonomy_stats(user_id=user_id)
...
def update_autonomy_settings():
    ...
    try:
        ...
        autonomy_manager.set_user_autonomy_level(user_id=get_current_user_id(), level=new_level)
...
def get_action_log():
    ...
    try:
        ...
        actions = conn.execute('''
            ...
            LIMIT ?
        ''', (get_current_user_id(), limit)).fetchall()
    ...
    try:
        ...
        return jsonify({
            ...
            'recent_performance': {
                ...
                'avg_impact': round(recent_actions['avg_impact'], 2)
                if recent_actions and recent_actions['avg_impact'] else 0,
...
def check_director():
    ...
    try:
        ...
        result = director_service.check_user_velocity(user_id=get_current_user_id())
        return jsonify(result)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def trigger_shadow_duel():
    ...
    try:
        ...
        result = doppelganger_service.generate_shadow_duel(user_id=get_current_user_id())
        return jsonify(result)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
