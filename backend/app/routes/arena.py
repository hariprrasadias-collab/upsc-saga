...
def get_available_bosses():
    ...
    # Year Bosses (Optimized: fetch count in a single query)
    years = conn.execute(
        "SELECT year, COUNT(*) as count FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats(
        'YEAR', row['year'], precomputed_count=row['count']) for row in years]

    # Subject Bosses (Optimized: fetch count in a single query)
    subjects = conn.execute(
        "SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats(
        'SUBJECT', row['subject'], precomputed_count=row['count']) for row in subjects]
...
def start_fight():
    try:
        ...
        if boss_type == 'YEAR':
            rows = conn.execute(
                "SELECT * FROM pyq_questions WHERE year = ? ORDER BY RANDOM() LIMIT ?", (boss_id, boss['hp'])).fetchall()
        elif boss_type == 'SUBJECT':
            rows = conn.execute(
                "SELECT * FROM pyq_questions WHERE subject = ? ORDER BY RANDOM() LIMIT ?", (boss_id, boss['hp'])).fetchall()
        elif boss_type == 'CUSTOM':
            # Fetch filters again
            boss_row = conn.execute(
                "SELECT * FROM custom_bosses WHERE id = ?", (boss_id,)).fetchone()
