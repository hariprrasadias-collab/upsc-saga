...
def get_boss_stats(boss_type, boss_id, pre_count=None, pre_row=None):
    """Generate boss stats dynamically based on DB content.
       Optimized to accept pre_count or pre_row to avoid N+1 queries.
    """
    conn = get_db()
...
def get_available_bosses():
    ...
    # Year Bosses
    years = conn.execute("SELECT year, COUNT(*) as cnt FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], pre_count=row['cnt']) for row in years]
    
    # Subject Bosses
    subjects = conn.execute("SELECT subject, COUNT(*) as cnt FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], pre_count=row['cnt']) for row in subjects]
    ...
    for row in custom:
        try:
            custom_bosses.append(get_boss_stats('CUSTOM', row['id'], pre_row=row))
        except Exception:
            continue
