...
def get_boss_stats(boss_type, boss_id, count=None):
...
def get_available_bosses():
    ...
    # Year Bosses
    # Bolt Optimization: Eliminating N+1 query issue by pre-calculating the counts for all years in a single GROUP BY query,
    # rather than making a separate COUNT(*) database request inside the get_boss_stats loop for every individual year.
    years = conn.execute(
        "SELECT year, COUNT(*) as count FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], count=row['count']) for row in years]

    # Subject Bosses
    # Bolt Optimization: Applying the same N+1 optimization by pre-calculating counts for all subjects using a single query.
    subjects = conn.execute(
        "SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], count=row['count']) for row in subjects]
...
def start_fight():
    try:
        ...
        if boss_type == 'YEAR':
            rows = conn.execute("SELECT * FROM pyq_questions WHERE year = ? ORDER BY RANDOM() LIMIT ?",
                                (boss_id, boss['hp'])).fetchall()
        elif boss_type == 'SUBJECT':
            rows = conn.execute("SELECT * FROM pyq_questions WHERE subject = ? ORDER BY RANDOM() LIMIT ?",
                                (boss_id, boss['hp'])).fetchall()
        elif boss_type == 'CUSTOM':
            # Fetch filters again
            boss_row = conn.execute("SELECT * FROM custom_bosses WHERE id = ?", (boss_id,)).fetchone()
