...
def get_available_bosses():
    ...
    # Year Bosses - Calculate counts in bulk to prevent N+1 queries
    year_counts = conn.execute("SELECT year, COUNT(*) as count FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], precalculated_count=row['count']) for row in year_counts]
    
    # Subject Bosses - Calculate counts in bulk to prevent N+1 queries
    subject_counts = conn.execute("SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], precalculated_count=row['count']) for row in subject_counts]
