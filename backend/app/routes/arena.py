...
def create_custom_boss():
    ...
    try:
        ...
        conn = get_db()
        cursor = conn.execute(
            'INSERT INTO custom_bosses (name, filters) VALUES (?, ?)',
            (name,
             json.dumps(filters)))
        conn.commit()
...
def get_available_bosses():
    ...
    # Year Bosses - Optimized to prevent N+1 queries by grouping counts
    years = conn.execute(
    ...
    # Subject Bosses - Optimized to prevent N+1 queries by grouping counts
    subjects = conn.execute(
        "SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    # Handle both sqlite3.Row dict-like access and tuple access depending on
    # the row factory
    subject_bosses = []
    for row in subjects:
        try:
