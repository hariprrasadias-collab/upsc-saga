...
def get_boss_stats(boss_type, boss_id, precomputed_count=None):
    ...
    Performance: precomputed_count prevents N+1 queries when fetching lists of bosses.
    """
    conn = get_db()
...
def get_available_bosses():
    ...
    # Year Bosses
    # Optimized: Fetches all year counts simultaneously using GROUP BY, avoiding an N+1 query loop.
    ...
    # Subject Bosses
    # Optimized: Fetches all subject counts simultaneously using GROUP BY, avoiding an N+1 query loop.
