...
def get_boss_stats(boss_type, boss_id, pre_counted_max_hp=None):
    """Generate boss stats dynamically based on DB content"""
    conn = get_db()
    
    if boss_type == 'YEAR':
        # Boss ID is the Year (e.g., 2024)
        count = pre_counted_max_hp if pre_counted_max_hp is not None else conn.execute("SELECT COUNT(*) FROM pyq_questions WHERE year = ?", (boss_id,)).fetchone()[0]
        name = f"The {boss_id} Titan"
        loot = ["Time Capsule", "Ancient Scroll"]
    elif boss_type == 'SUBJECT':
        # Boss ID is the Subject Name (e.g., Geography)
        count = pre_counted_max_hp if pre_counted_max_hp is not None else conn.execute("SELECT COUNT(*) FROM pyq_questions WHERE subject = ?", (boss_id,)).fetchone()[0]
...
def get_available_bosses():
    ...
    # Year Bosses
    year_counts = conn.execute("SELECT year, COUNT(*) as c FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], pre_counted_max_hp=row['c']) for row in year_counts]
    
    # Subject Bosses
    subject_counts = conn.execute("SELECT subject, COUNT(*) as c FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], pre_counted_max_hp=row['c']) for row in subject_counts]
