...
def get_boss_stats(boss_type, boss_id, precalc_count=None):
    ...
    if boss_type == 'YEAR':
        ...
        count = precalc_count if precalc_count is not None else conn.execute(
...
def get_available_bosses():
    ...
    # Year Bosses
    # ⚡ Bolt Optimization: Use GROUP BY to get counts in a single query (O(1)) instead of N+1 queries.
    years = conn.execute(
        "SELECT year, COUNT(*) as count FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], precalc_count=row['count']) for row in years]

    # Subject Bosses
    # ⚡ Bolt Optimization: Use GROUP BY to get counts in a single query (O(1)) instead of N+1 queries.
    subjects = conn.execute(
        "SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], precalc_count=row['count']) for row in subjects]
