...
def get_available_bosses():
    ...
    # Year Bosses - using single query to avoid N+1 queries
    year_counts = conn.execute("SELECT year, COUNT(*) as count FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], precalc_count=row['count']) for row in year_counts]
    
    # Subject Bosses - using single query to avoid N+1 queries
    subject_counts = conn.execute("SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], precalc_count=row['count']) for row in subject_counts]
