...
def get_available_bosses():
    ...
    # Year Bosses
    years = conn.execute("SELECT year, COUNT(*) as hp FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], precalc_count=row['hp']) for row in years]
    
    # Subject Bosses
    subjects = conn.execute("SELECT subject, COUNT(*) as hp FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], precalc_count=row['hp']) for row in subjects]
