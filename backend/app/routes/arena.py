...
def get_boss_stats(boss_type, boss_id, precomputed_count=None, custom_row=None):
    ...
    if boss_type == 'YEAR':
        ...
        boss_row = custom_row if custom_row is not None else conn.execute("SELECT * FROM custom_bosses WHERE id = ?", (boss_id,)).fetchone()
        if not boss_row:
...
def get_available_bosses():
    ...
    for row in custom:
        try:
            custom_bosses.append(get_boss_stats('CUSTOM', row['id'], custom_row=row))
        except Exception:
            continue
