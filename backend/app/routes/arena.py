...
def get_boss_stats(boss_type, boss_id, precomputed_count=None):
    ...
    ⚡ Bolt Optimization: Accepts precomputed_count to eliminate N+1 queries.
    """
    conn = get_db()
...
def get_available_bosses():
    ...
    year_bosses = [
        get_boss_stats(
            'YEAR',
            row['year'],
            precomputed_count=row['count']) for row in years if row['year'] is not None]
    ...
    subject_bosses = [
        get_boss_stats(
            'SUBJECT',
            row['subject'],
            precomputed_count=row['count']) for row in subjects if row['subject'] is not None]
