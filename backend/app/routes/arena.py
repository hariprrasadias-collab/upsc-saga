...
def get_available_bosses():
    ...
    year_bosses = [get_boss_stats('YEAR', row['year'], precomputed_count=row['count']) for row in year_counts]

    # Subject Bosses
    subject_counts = conn.execute(
        "SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], precomputed_count=row['count'])
