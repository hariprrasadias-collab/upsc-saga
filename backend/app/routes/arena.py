...
def get_available_bosses():
    ...
    # Year Bosses
    years = conn.execute("SELECT year, COUNT(*) as c FROM pyq_questions GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], precalculated_count=row['c']) for row in years]
    
    # Subject Bosses
    subjects = conn.execute("SELECT subject, COUNT(*) as c FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], precalculated_count=row['c']) for row in subjects]
