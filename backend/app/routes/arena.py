...
def get_boss_stats(boss_type, boss_id, precalculated_count=None):
    ...
    if boss_type == 'YEAR':
        ...
        count = precalculated_count if precalculated_count is not None else conn.execute(query, params).fetchone()[0]
    else:
        # Default/Random Boss
        count = 10
        name = "Training Dummy"
...
def get_available_bosses():
    ...
    # ⚡ Bolt Optimization: Use GROUP BY to fetch counts in bulk, avoiding N+1 queries
