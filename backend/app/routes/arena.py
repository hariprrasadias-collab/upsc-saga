...
def get_available_bosses():
    ...
    year_bosses = [get_boss_stats('YEAR', row['year'], row['c']) for row in year_counts]
    
    # Subject Bosses
    subject_counts = conn.execute("SELECT subject, COUNT(*) as c FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], row['c']) for row in subject_counts]
