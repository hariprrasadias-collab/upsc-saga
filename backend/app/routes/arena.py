...
def get_boss_stats(boss_type, boss_id, pre_count=None, pre_row=None):
    ...
    if boss_type == 'YEAR':
        ...
        # Default/Random Boss
        count = pre_count if pre_count is not None else 10
        name = "Training Dummy"
        loot = ["Wooden Sword"]
...
def get_available_bosses():
    ...
    year_bosses = [get_boss_stats('YEAR', row['year'], pre_count=row['count']) for row in years if row['year']]
    
    # Subject Bosses
    subjects = conn.execute("SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], pre_count=row['count']) for row in subjects if row['subject']]
    ...
    for row in custom:
        try:
            # We pass pre_row but we don't pre_count because custom bosses have complex dynamic filters
