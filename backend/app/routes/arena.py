...
def get_available_bosses():
    ...
    # Year Bosses
    years = conn.execute("SELECT year, COUNT(*) as cnt FROM pyq_questions WHERE year IS NOT NULL GROUP BY year ORDER BY year DESC").fetchall()
    year_bosses = [get_boss_stats('YEAR', row['year'], precalculated_count=row['cnt']) for row in years]
    
    # Subject Bosses
    subjects = conn.execute("SELECT subject, COUNT(*) as cnt FROM pyq_questions WHERE subject IS NOT NULL GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = [get_boss_stats('SUBJECT', row['subject'], precalculated_count=row['cnt']) for row in subjects]
