...
def get_boss_stats(boss_type, boss_id, precomputed_count=None):
    ...
    if boss_type == 'YEAR':
        ...
        count = precomputed_count if precomputed_count is not None else conn.execute(
            "SELECT COUNT(*) FROM pyq_questions WHERE year = ?", (boss_id,)).fetchone()[0]
        name = f"The {boss_id} Titan"
        loot = ["Time Capsule", "Ancient Scroll"]
    elif boss_type == 'SUBJECT':
        # Boss ID is the Subject Name (e.g., Geography)
        count = precomputed_count if precomputed_count is not None else conn.execute(
            "SELECT COUNT(*) FROM pyq_questions WHERE subject = ?", (boss_id,)).fetchone()[0]
        ...
        boss_row = conn.execute(
            "SELECT * FROM custom_bosses WHERE id = ?", (boss_id,)).fetchone()
        if not boss_row:
...
def get_available_bosses():
    ...
    # ⚡ Bolt Optimization: Prevent N+1 query loop for Year and Subject bosses.
    # Impact: Reduces queries from O(N) where N is (Years + Subjects) to just 2 O(1) queries.
    # Measurement: The /bosses endpoint responds much faster without iterative count lookups.

    # Year Bosses
    years_counts = conn.execute(
    ...
    for row in years_counts:
        try:
            ...
            y = row[0]
            c = row[1]
        year_bosses.append(get_boss_stats('YEAR', y, precomputed_count=c))

    # Subject Bosses
    subjects_counts = conn.execute(
        "SELECT subject, COUNT(*) as count FROM pyq_questions GROUP BY subject ORDER BY subject").fetchall()
    subject_bosses = []
    for row in subjects_counts:
        try:
            s = row['subject']
            c = row['count']
        except (TypeError, IndexError):
            s = row[0]
            c = row[1]
        subject_bosses.append(get_boss_stats(
            'SUBJECT', s, precomputed_count=c))

    # Custom Bosses
    custom = conn.execute(
        "SELECT id FROM custom_bosses WHERE is_active = 1 ORDER BY created_at DESC").fetchall()
    custom_bosses = []
    for row in custom:
        try:
...
def start_fight():
    try:
        ...
        if boss_type == 'YEAR':
            rows = conn.execute(
                "SELECT * FROM pyq_questions WHERE year = ? ORDER BY RANDOM() LIMIT ?",
                (boss_id, boss['hp'])
            ).fetchall()
        elif boss_type == 'SUBJECT':
            rows = conn.execute(
                "SELECT * FROM pyq_questions WHERE subject = ? ORDER BY RANDOM() LIMIT ?",
                (boss_id, boss['hp'])
            ).fetchall()
        elif boss_type == 'CUSTOM':
            # Fetch filters again
