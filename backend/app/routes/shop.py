...
# Define the Shop Catalog (Source of Truth)
SHOP_CATALOG = {
...
def get_inventory():
    ...
    conn = get_db()
    items = conn.execute(
        'SELECT item_id, equipped FROM inventory WHERE user_id = ?',
        (user_id,)
    ).fetchall()
    user = conn.execute(
        'SELECT hacksilver FROM users WHERE id = ?',
        (user_id,)
    ).fetchone()
    return jsonify({
...
def buy_item():
    ...
    conn.execute(
        ...
        '(user_id, item_id, item_name, equipped) VALUES (?, ?, ?, 0)',
    ...
    return jsonify({
        ...
        "new_balance": user['hacksilver'] - cost
    }), 200
