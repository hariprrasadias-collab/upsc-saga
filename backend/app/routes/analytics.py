...
def get_overview():
    ...
    try:
        ...
        _end_date = datetime.now().isoformat()

        # Study hours
        study_hours = calculate_study_hours(
            conn, user_id, start_date, _end_date)
        ...
        try:
            mock_count = conn.execute(
                'SELECT COUNT(*) FROM test_attempts WHERE user_id = ? AND submitted_at >= ?',
        ...
        try:
            answer_count = conn.execute(
                'SELECT COUNT(*) FROM user_answers WHERE user_id = ? AND submitted_at >= ?',
                (user_id, start_date)).fetchone()[0]
        except Exception:
            answer_count = 0
        ...
        try:
            review_count = conn.execute(
                'SELECT COUNT(DISTINCT flashcard_id) FROM review_sessions '
...
def get_subject_wise():
    ...
    try:
        ...
        # Bolt optimization: Bulk query subject performances to avoid N+1 subqueries per subject
