    ...
    try:
        ...
        try:
            review_count = conn.execute(
                'SELECT COUNT(DISTINCT flashcard_id) FROM review_sessions WHERE user_id = ? AND reviewed_at >= ?',
                (user_id,
                 start_date)).fetchone()[0]
        except Exception:
            review_count = 0
        ...
        try:
            task_count = conn.execute(
                'SELECT COUNT(*) FROM tasks WHERE user_id = ? AND isCompleted = 1',
                (user_id,
                 )).fetchone()[0]
        except Exception:
            task_count = 0
...
def get_mock_test_analytics():
    ...
    try:
        ...
        return jsonify({'trends': [], 'improvement_rate': 0,
                       'subject_stats': []}), 200
...
def get_progress_trend():
    ...
    try:
        ...
        if metric == 'syllabus':
            ...
            # so we only need to calculate the current progress once instead of
            # (days + 1) times.
