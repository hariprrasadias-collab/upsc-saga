    ...
    try:
        ...
        try:
            answer_scores = conn.execute(f'''
                ...
                WHERE ua.user_id = ? AND aq.subject IN ({','.join(['?']*len(subjects))})
                GROUP BY aq.subject
            ''', (user_id, *subjects)).fetchall()
        ...
        try:
            syllabus_scores = conn.execute(f'''
                SELECT
                    subject,
                    COUNT(*) as total,
                    SUM(CASE WHEN status = 'Completed' THEN 1 ELSE 0 END) as completed
                FROM syllabus_topics
                WHERE subject IN ({','.join(['?']*len(subjects))})
                GROUP BY subject
            ''', subjects).fetchall()
            for row in syllabus_scores:
                if row['total'] > 0:
