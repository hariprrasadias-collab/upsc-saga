...
def get_time_distribution():
    ...
    try:
        ...
        # Group by date using SQL to offload workload and reduce data transfer
        dates = conn.execute('''
        ...
        heatmap_data = [
            {'date': row['date'], 'hours': min(row['activity_count'] * 0.5, 8)}  # Estimate: 30min per activity, cap at 8h
            for row in dates
        ]
