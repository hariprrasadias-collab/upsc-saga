...
def get_time_distribution():
    ...
    try:
        ...
        # Bolt Optimization: Shifted counting workload to DB via GROUP BY to eliminate O(N) memory transfer
        # and iterations over all individual activity records.
        # Group by date
        dates = conn.execute('''
        ...
        # Count activities per date directly from grouped counts
        for row in dates:
            date = row['date']
            activities[date] = row['count']
