...
def get_time_distribution():
    ...
    try:
        ...
        # Bolt Optimization: Shifted O(N) aggregation workload to the database using GROUP BY date
        # instead of fetching raw rows via UNION ALL and counting them in Python.
        # Group by date
        dates = conn.execute('''
        ...
        for row in dates:
            activities[row['date']] = activities.get(row['date'], 0) + row['count']
