    ...
    try:
        ...
        try:
            mock_count = conn.execute(
                'SELECT COUNT(*) FROM test_attempts WHERE user_id = ? AND submitted_at >= ?', (user_id, start_date)).fetchone()[0]
        except Exception:
            mock_count = 0

        try:
            answer_count = conn.execute(
                'SELECT COUNT(*) FROM user_answers WHERE user_id = ? AND submitted_at >= ?', (user_id, start_date)).fetchone()[0]
        except Exception:
            answer_count = 0

        try:
            review_count = conn.execute(
                'SELECT COUNT(DISTINCT flashcard_id) FROM review_sessions WHERE user_id = ? AND reviewed_at >= ?', (user_id, start_date)).fetchone()[0]
        except Exception:
            review_count = 0

        try:
            task_count = conn.execute(
                'SELECT COUNT(*) FROM tasks WHERE user_id = ? AND isCompleted = 1', (user_id,)).fetchone()[0]
        except Exception:
            task_count = 0
...
def get_subject_wise():
    ...
    try:
        ...
        # Batched performance fetch to eliminate N+1 queries
        results = get_all_subject_performance(conn, user_id, subjects)
...
def get_time_distribution():
    ...
    try:
        ...
        heatmap_data = [
            # Estimate: 30min per activity, cap at 8h
            {'date': date, 'hours': min(count * 0.5, 8)}
...
def get_progress_trend():
    ...
    try:
        ...
        if metric == 'syllabus':
            ...
            total = conn.execute(
                'SELECT COUNT(*) FROM syllabus_topics').fetchone()[0]
            ...
            trend_data = [{'date': s['date'], 'value': round(
                s['avg_score'], 1)} for s in scores]
