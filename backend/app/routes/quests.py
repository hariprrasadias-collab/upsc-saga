    ...
    if request.method == 'GET':
        ...
        if not quests:
            try:
                ...
                # ⚡ Bolt Optimization: Use bulk insert to avoid N+1 query problem during daily quest generation
                quest_data = [(user_id, q['title'], q['xp_reward'], q['type']) for q in new_quests]
                if quest_data:
                    conn.executemany('INSERT INTO tasks (user_id, title, xp_reward, associated_stat, isCompleted, is_quest) VALUES (?, ?, ?, ?, 0, 1)', quest_data)
                conn.commit()
                # Fetch again
                quests = conn.execute('SELECT * FROM tasks WHERE user_id=? AND is_quest=1 ORDER BY isCompleted ASC', (user_id,)).fetchall()
