    ...
    if request.method == 'GET':
        ...
        if not quests:
            try:
                ...
                quest_params = [(user_id, q['title'], q['xp_reward'], q['type']) for q in new_quests]
                if quest_params:
                    # ⚡ Bolt: Use executemany to fix N+1 query pattern during bulk quest generation
                    conn.executemany('INSERT INTO tasks (user_id, title, xp_reward, associated_stat, isCompleted, is_quest) VALUES (?, ?, ?, ?, 0, 1)',
                                     quest_params)
                conn.commit()
                # Fetch again
                quests = conn.execute('SELECT * FROM tasks WHERE user_id=? AND is_quest=1 ORDER BY isCompleted ASC', (user_id,)).fetchall()
