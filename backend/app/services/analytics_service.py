...
def identify_weak_areas(conn, user_id, limit=10):
    ...
    try:
        ...
        if low_scores:
            # Fetch all required scores in a single bulk query (N+1 optimization)
            subjects = [subj['subject'] for subj in low_scores]
            placeholders = ','.join(['?'] * len(subjects))

            bulk_scores = conn.execute(f'''
            ...
            for row in bulk_scores:
                grouped_scores[row['subject']].append({
                    'score': row['score'],
                    'submitted_at': row['submitted_at']
                })

            for subj in low_scores:
                subject_scores = grouped_scores.get(subj['subject'], [])
