...
def identify_weak_areas(conn, user_id, limit=10):
    ...
    try:
        ...
        if low_scores:
            subjects_list = [s['subject'] for s in low_scores]
            ...
            ''', [user_id] + subjects_list).fetchall()

            subject_to_scores = {s: [] for s in subjects_list}
            for row in all_scores:
                subject_to_scores[row['subject']].append(row)

            for subj in low_scores:
                subject_scores = subject_to_scores.get(subj['subject'], [])
