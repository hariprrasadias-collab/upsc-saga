...
def get_all_subject_performances(conn, user_id, subjects):
    """
    Aggregate metrics for multiple subjects simultaneously to avoid N+1 queries.
    """
    results_map = {
        subject: {
            'subject': subject,
    ...
    try:
        ...
        mock_avgs = conn.execute(mock_query, params).fetchall()
        ...
        answer_query = f'''
            ...
            GROUP BY aq.subject
        '''
        answer_avgs = conn.execute(answer_query, params).fetchall()
        ...
        syllabus_query = f'''
            ...
            GROUP BY subject
        '''
        syllabus_stats = conn.execute(syllabus_query, subjects).fetchall()
        for row in syllabus_stats:
            if row['total'] > 0:
                results_map[row['subject']]['syllabus_pct'] = round((row['completed'] / row['total']) * 100, 1)

    except Exception:
        pass # Fallback to initialized 0s on error

    return [results_map[subject] for subject in subjects]
