...
def get_all_subjects_performance(conn, user_id, subjects):
    """
    Aggregate all metrics for multiple subjects in batch
    Bolt Optimization: Replaces N+1 queries with bulk IN clauses and GROUP BY.
    """
    if not subjects:
        return []
    ...
    try:
        ...
        mock_data = conn.execute(mock_query, params).fetchall()
    ...
    try:
        ...
        '''
        params = [user_id] + subjects
        answer_data = conn.execute(answer_query, params).fetchall()
    ...
    try:
        ...
        '''
        params = subjects
        syllabus_data = conn.execute(syllabus_query, params).fetchall()
