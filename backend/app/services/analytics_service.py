...
def get_bulk_subject_performance(conn, user_id, subjects):
    """
    Aggregate all metrics for specific subjects in bulk
    """
    results = {subj: {
        'subject': subj,
        'mock_avg': 0,
...
def identify_weak_areas(conn, user_id, limit=10):
    ...
    try:
        ...
        for top in syllabus_weak:
            weak_areas.append({
                'subject': top['subject'],
                'topic': f"{top['topic']}",
                'weakness_score': 80 if top['status'] == 'Not Started' else 50,
                'source': 'Syllabus',
                'action': 'Start reading' if top['status'] == 'Not Started' else 'Complete reading'
            })
        ...
        if low_scores:
            subjects = [subj['subject'] for subj in low_scores]
            placeholders = ','.join(['?'] * len(subjects))

            # Get all scores for these subjects in one go
            ...
            # Group by subject
            scores_by_subj = {subj: [] for subj in subjects}
            for row in all_scores:
                scores_by_subj[row['subject']].append(row)

            for subj in low_scores:
                subject_name = subj['subject']
                subject_scores = scores_by_subj.get(subject_name, [])
