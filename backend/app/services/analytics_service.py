...
def get_all_subjects_performance(conn, user_id, subjects):
    """
    Aggregate metrics for multiple subjects to prevent N+1 queries.
    Uses bulk GROUP BY queries and maps the results in memory.
    """
    results = {
        subject: {
            'subject': subject,
    ...
    # Mock tests
    try:
        mock_params = [user_id] + list(subjects)
        mock_avg_rows = conn.execute(f'''
        ...
        for row in mock_avg_rows:
            if row['avg_score']:
                results[row['subject']]['mock_avg'] = round(row['avg_score'], 1)
    except Exception as e:
        print(f"Error fetching bulk mock performance: {e}")

    # Answer writing
    try:
        answer_params = [user_id] + list(subjects)
        answer_avg_rows = conn.execute(f'''
        ...
        for row in answer_avg_rows:
            if row['avg_score']:
                results[row['subject']]['answer_avg'] = round(row['avg_score'], 1)
    except Exception as e:
        print(f"Error fetching bulk answer writing performance: {e}")
