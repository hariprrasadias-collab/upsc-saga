...
def get_all_subject_performance(conn, user_id, subjects):
    """
    Aggregate all metrics for a list of subjects efficiently (preventing N+1 queries).
    Executes a single query per metric grouping by subject.
    """
    results = {
        subj: {
            'subject': subj,
    ...
    try:
        ...
        print(f"Error fetching mock test performance: {e}")
    ...
    try:
        ...
        print(f"Error fetching answer writing performance: {e}")
    ...
    try:
        ...
        print(f"Error fetching syllabus performance: {e}")
    
    # Return in original requested order to maintain consistency
