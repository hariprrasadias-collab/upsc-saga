...
def identify_weak_areas(conn, user_id, limit=10):
    ...
    try:
        ...
        # Batch fetch all historical scores to avoid N+1 query bottleneck
        subject_names = [subj['subject'] for subj in low_scores]
        all_subject_scores = defaultdict(list)
        ...
        if subject_names:
            ...
            '''
            params = [user_id] + subject_names
            results = conn.execute(query, params).fetchall()

            for row in results:
                all_subject_scores[row['subject']].append(row)

        for subj in low_scores:
            # Calculate trend for this subject using in-memory grouped scores
            subject_scores = all_subject_scores.get(subj['subject'], [])
