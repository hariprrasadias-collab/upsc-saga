...
def identify_weak_areas(conn, user_id, limit=10):
    ...
    try:
        ...
        # Bolt optimization: Extract subjects and fetch all historical scores in one query (avoid N+1)
        if low_scores:
            import collections
            ...
            # Group scores by subject in memory
            scores_by_subject = collections.defaultdict(list)
            for row in all_scores:
                scores_by_subject[row['subject']].append(row)
            ...
            for subj in low_scores:
                ...
                # Handle possible SQLite Row type issues dynamically as per memory constraints
                last_attempt = None
                if subject_scores:
                    try:
                        last_attempt = subject_scores[-1]['submitted_at']
                    except (TypeError, IndexError):
                        last_attempt = subject_scores[-1][2]  # Index 2 is submitted_at
