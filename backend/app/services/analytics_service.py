...
def get_all_subject_performance(conn, user_id, subjects):
    """
    Aggregate all metrics for multiple subjects efficiently without N+1 queries.
    """
    results_map = {
        subject: {
            'subject': subject,
    ...
    try:
        ...
        mock_rows = conn.execute(mock_query, mock_params).fetchall()
    ...
    try:
        ...
        answer_rows = conn.execute(answer_query, answer_params).fetchall()
    ...
    # Return exactly in the order of the requested subjects
