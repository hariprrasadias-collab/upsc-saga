...
def get_bulk_subject_performance(conn, user_id, subjects):
    """
    Aggregate all metrics for a list of subjects in bulk to prevent N+1 queries.
    """
    results = {
        subject: {
            'subject': subject,
    ...
    try:
        ...
        mock_results = conn.execute(mock_query, params).fetchall()
    ...
    try:
        ...
        '''
        params = [user_id] + subjects
        answer_results = conn.execute(answer_query, params).fetchall()
    ...
    try:
        ...
        '''
        params = subjects
        syllabus_results = conn.execute(syllabus_query, params).fetchall()
