...
def get_all_subjects_performance(conn, user_id, subjects):
    """
    Bolt Optimization: Batch analytics queries for all subjects at once
    to avoid N+1 queries.
    Impact: O(N) queries to O(1) batched queries (exactly 5 queries).
    """
    results = {s: {
        'subject': s,
        'mock_avg': 0,
    ...
    try:
        flashcard_stats = conn.execute(f'''
            ...
            WHERE rs.user_id = ? AND d.subject IN ({placeholders}) AND rs.halflife >= 180
            GROUP BY d.subject
        ''', [user_id] + subjects).fetchall()
