...
def get_subject_performance(conn, user_id, subject, precalc_data=None):
    ...
    Accepts optional precalc_data to avoid N+1 queries.
    """
    result = {
        'subject': subject,
        'mock_avg': 0,
    ...
    if precalc_data is not None:
        if precalc_data['mock_avg'].get(subject):
            result['mock_avg'] = round(precalc_data['mock_avg'][subject], 1)
        if precalc_data['answer_avg'].get(subject):
            result['answer_avg'] = round(precalc_data['answer_avg'][subject], 1)

        syl_data = precalc_data['syllabus'].get(subject)
        if syl_data and syl_data['total'] > 0:
            result['syllabus_pct'] = round((syl_data['completed'] / syl_data['total']) * 100, 1)
    else:
        try:
            # Mock tests
