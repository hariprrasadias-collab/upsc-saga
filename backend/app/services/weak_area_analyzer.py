...
def analyze_all_performance() -> List[Dict]:
    ...
    ⚡ Bolt Optimization: Uses bulk GROUP BY queries and executemany UPSERT
    to prevent N+1 nested subquery bottlenecks when analyzing all topics.
    """
    conn = get_db()
    cursor = conn.cursor()
    
    # Bulk get performance stats for all topics
    cursor.execute('''
        SELECT
            topic,
            COUNT(*) as total,
    ...
    # Bulk get recent failures for all topics
    cursor.execute('''
