...
def analyze_all_performance() -> List[Dict]:
    ...
    # Get overall stats for all topics in one query
    cursor.execute('''
        SELECT
            topic,
            COUNT(*) as total,
    ...
    # Get recent failures for all topics in another query
    cursor.execute('''
        ...
        GROUP BY topic
    ''')
    recent_failures_map = {row['topic']: row['recent_failures'] for row in cursor.fetchall()}
    ...
    for row in topics_data:
        ...
        updates.append((
            topic, subject, total, correct, accuracy, avg_time, weakness_score
        ))
    ...
    if updates:
        # Bulk update the weak_areas table
        cursor.executemany('''
            INSERT INTO weak_areas
            (topic, subject, total_attempts, correct_attempts, accuracy_rate, avg_time_taken, weakness_score, last_updated)
            VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
