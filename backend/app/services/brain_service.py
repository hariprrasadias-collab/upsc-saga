...
class BrainService:
    ...
    def process_task_completion(self, task_data: dict):
        ...
        try:
            ...
            # 13. Neural Hash Decoding (Examiner's Lens)
            # Use synthesis from Triangulation if available, else standard text
            synthesis_text = ""
            if triangulation_res.get('success'):
            ...
            if nh_res.get('success'):
                ...
                if deck_row:
                    ...
                    conn.execute('''
                        INSERT INTO flashcards (deck_id, front, back, source)
                        VALUES (?, ?, ?, 'ai_generated_neural_hash')
                    ''', (deck_id, f"Examiner's Lens: {topic}", content,))
                    conn.commit()

            # 14. Mistake Pattern Detection (Common Pitfalls)
            # We use a new action type FIND_COMMON_PITFALLS which we will implement below
            ...
            if pitfall_res.get('success'):
                ...
                if pitfalls:
                    ...
                    if deck_row:
                        ...
                        conn.execute('''
                            INSERT INTO flashcards (deck_id, front, back, source)
                            VALUES (?, ?, ?, 'ai_generated_pitfalls')
                        ''', (deck_id, f"Common Pitfalls: {topic}", content,))
                        conn.commit()

            # 15. Podcast Script Generation (Audio Learning)
            ...
            if podcast_res.get('success'):
                ...
                if deck_row:
                    ...
                    conn.execute('''
                        INSERT INTO flashcards (deck_id, front, back, source)
                        VALUES (?, ?, ?, 'ai_generated_podcast')
                    ''', (deck_id, f"Podcast Script: {topic}", podcast_res.get('script'),))
                    conn.commit()

            # 16. Feynman Challenge (Teach-Back)
            # Schedule a task for the user to explain the topic
            try:
                from app.db import get_db
                conn = get_db()
                from datetime import timedelta
                ...
                if not exists:
                    conn.execute('''
                        INSERT INTO tasks (user_id, title, due_date, xp_reward, associated_stat, isCompleted, is_quest)
                        VALUES (?, ?, ?, ?, ?, 0, 0)
                    ''', (1, check_title, check_date, 75, 'Communication'))
                    conn.commit()
            ...
            if essay_res.get('success'):
                ...
                if prompt_text:
                    ...
                    if deck_row:
                        ...
                        conn.execute('''
                            INSERT INTO flashcards (deck_id, front, back, source)
                            VALUES (?, ?, ?, 'ai_generated_essay')
                        ''', (deck_id, f"Essay Prompt: {topic}", prompt_text,))
                        conn.commit()
            ...
            if visual_res.get('success'):
                ...
                if prompt_text:
                    ...
                    if deck_row:
                        ...
                        conn.execute('''
                            INSERT INTO flashcards (deck_id, front, back, source)
                            VALUES (?, ?, ?, 'ai_generated_visual')
                        ''', (deck_id, f"Visual Mnemonic Prompt: {topic}", prompt_text,))
                        conn.commit()

            # 19. Check for Book Completion -> Trigger Boss Fight
